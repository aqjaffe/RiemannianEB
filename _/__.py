    ...
    G_sampler_ls = [
        get_G_class(manifold_type, sampler, name, params) for sampler, name, params 
            in [
                ...
                (multimodal_sampler, '2-modal -', {'tau2' : 0.0001, 'num_modes' : 2}),
                (multimodal_sampler, '2-modal --', {'tau2' : 0.3, 'num_modes' : 2}),
                # (multimodal_sampler, '3-modal', {'tau2' : 0.05, 'num_modes' : 3}),
                (multimodal_sampler, '4-modal', {'tau2' : 0.025, 'num_modes' : 4}),
                (multimodal_sampler, '8-modal', {'tau2' : 0.005, 'num_modes' : 8}),
            ]
        ]

if manifold_type == 'S2':
    n_samples_ls =  np.round(np.logspace(np.log10(1000), np.log10(10000), 5)).astype(int)
    ...
    G_sampler_ls = [
        get_G_class(manifold_type, sampler, name, params) for sampler, name, params 
            in [
                ...
                (multimodal_sampler, '6-modal', {'tau2' : 0.01, 'num_modes' : 6}),
                (multimodal_sampler, '12-modal', {'tau2' : 0.001, 'num_modes' : 12}),
            ]
        ]
...
def run_one_mc(imc, manifold_type, G, n_samples_ls, M_grid, rho_grid, sigma2,
                test_Theta, test_X, oracle_loss_val, naive_loss_val, oracle_delta, cv):
    ...
    for ixn, n_samples in enumerate(n_samples_ls):
        ...
        if cv:
            M_star, rho_star = scoreMatchingKFoldCV(
                manifold_type, train_X, M_grid, n_splits=5, display_tqdm=False)['AIC']
            # M_star, rho_star = scoreMatching(manifold_type, train_X, M_grid, n_splits=5, display_tqdm=False)['AIC']
            ...
            cv_excess_loss[ixn]   = emp_loss[ixn, ixM_star, ixrho_star] - oracle_loss_val
...
def converenge_rate_experiment(manifold_type, G, n_samples_ls, M_grid, rho_grid,
    ...
    for sigma2 in sigma2s:
        ...
        (imc, manifold_type, G, n_samples_ls, M_grid, rho_grid, sigma2,
                test_Theta, test_X, oracle_loss_val, naive_loss_val, oracle_delta, cv)
...
#     oracle_delta_T = oracle_denoiser(manifold_type, num_oracle_samples, sigma2, X, num_bins = 1000, G = G)
#     oracle_delta_T = oracle_denoiser__naive(manifold_type, num_oracle_samples, sigma2, X, G)
#     oracle_delta_T = oracle_denoiser__kernel(manifold_type, num_oracle_samples, sigma2, X, G)
