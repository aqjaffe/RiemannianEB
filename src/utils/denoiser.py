...
def denoiser(manifold_type, X, M, rho, sigma2, X_to_denoise, densityIn=None, k_modes=None):
    """
    ...
    Args:
        ...
        M: Spectral truncation degree (ignored when k_modes is set)
        rho: Regularization parameter to avoid division by zero
        sigma2: noise variance
        X_to_denoise: Points to denoise in extrinsic coordinates
        k_modes: Number of individual Fourier modes (T2 only); overrides M when set.
    ...
    if densityIn is None:
        _, hat_f, hat_grad_f = density_estimate(manifold_type, X, M, X_to_denoise, k_modes=k_modes)
    else:
