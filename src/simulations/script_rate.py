...
for manifold_type in ['S1','S2']:
    ...
    def run_one_mc(imc, manifold_type, G, n_samples_ls, M_grid, rho_grid, sigma2, test_size,
        ...
        naive_displacements = sq_loss(manifold, test_X, oracle_delta)

        if bayes:
            oracle_bayes_delta = oracle_bayes__kernel_cv(
                manifold_type, oracle_samples, sigma2, test_X
            )
            oracle_bayes_loss_val = sq_loss(manifold, oracle_bayes_delta, test_Theta)
        else:
            oracle_bayes_loss_val = float('nan')


        emp_loss      = np.zeros((len(n_samples_ls), len(M_grid), len(rho_grid)), dtype=float)
        displacements = np.zeros_like(emp_loss)
        excess_loss = np.zeros_like(emp_loss)
        ...
        if sigma2 == 0.15:
            for ixn, n_samples in enumerate(n_samples_ls):
                ...
                for ixM, M in enumerate(M_grid):
                    ...
                    for ixrho, rho_perc in enumerate(rho_grid):
                        positive_f = density_onX[0][density_onX[0] > 0]
                        rho = np.percentile(positive_f, rho_perc)
                        ...
                        excess_loss[ixn, ixM, ixrho]       = emp_loss[ixn, ixM, ixrho] - oracle_loss_val


        return {
            'naive_loss':            naive_loss_val,
            'oracle_loss':           oracle_loss_val,
            'oracle_bayes_loss':     oracle_bayes_loss_val,
            'emp_loss':              emp_loss,
            'displacements':         displacements,
            'excess_loss':           emp_loss - oracle_loss_val,
            'naive_displacements':   naive_displacements,
        }


    def converenge_rate_experiment(manifold_type, G, n_samples_ls, M_grid, rho_grid,
                                    sigma2s, test_size, num_oracle_samples, NMC, timenow, cv=False, bayes=False):
        ...
        for sigma2 in sigma2s:
            ...
            with tqdm_joblib(tqdm(
                ...
                mc_results = Parallel(n_jobs=-1)(
                    delayed(run_one_mc)(
                        ...
                        sigma2, test_size, oracle_samples, cv, bayes=bayes
                    )
            ...
            oracle_bayes_loss   = np.array([r['oracle_bayes_loss']    for r in mc_results])
            emp_loss            = np.stack([r['emp_loss']             for r in mc_results])  # (NMC, n, M, rho)
            displacements       = np.stack([r['displacements']        for r in mc_results])
            naive_displacements = np.stack([r['naive_displacements']  for r in mc_results])
            ...
            for ixn, n_samples in enumerate(n_samples_ls):
                all_oracleandcv_results.append({
                    ...
                    'mean_oracle_bayes_loss':    float(np.nanmean(oracle_bayes_loss)),
                    'std_oracle_bayes_loss':     float(np.nanstd(oracle_bayes_loss)),
                    'mean_naive_displacement':   float(np.mean(naive_displacements)),
                    'median_naive_displacement': float(np.median(naive_displacements)),
                    'std_naive_displacement':    float(naive_displacements.std()),
                })
    ...
    for G in G_sampler_ls:
        dfmc, dforaclecv = converenge_rate_experiment(
            ...
            sigma2s, test_size, num_oracle_samples, NMC, timenow, cv=True, bayes=True
        )
