...
\node[panel, text width=4cm] (p1) {
    \includegraphics[width=\imgw]{tikz_S1_1_part1.png}\\[1mm]
    \panelcaption{\textbf{1} Latent truths $\Theta \sim G$ on $S^1$}
};

\node[panel, text width=4cm, right=8mm of p1] (p2) {
    \includegraphics[width=\imgw]{tikz_S1_1_part2.png}\\[1mm]
    \panelcaption{\textbf{2} Observations $X \sim \mathcal{N}(\Theta, \sigma^2)$ }
};
...
\node[panel, text width=4cm, right=8mm of p34, yshift=0mm] (p5) {
